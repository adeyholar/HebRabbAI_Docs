...
/**
 * Nitro bundles `@electric-sql/pglite` into `_libs/` but does not copy the
 * sibling wasm/data files `import.meta.url` resolves. Local `vite preview`
 * (no DATABASE_URL) needs them next to the chunk; deployed Neon never loads
 * PGLite, so this is preview-only.
 */
function copyPglitePreviewAssets(root: string) {
...
export default defineConfig(({ command, isPreview }) => {
  if (isPreview) {
    try {
      copyPglitePreviewAssets(process.cwd());
    } catch (err) {
      console.error("[app-builder] PGLite preview assets:", err);
    }
  }
  return {
  server: {
